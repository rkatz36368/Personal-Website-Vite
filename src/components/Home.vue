...
<div id='app'>
  ...
  <v-btn id="buttonToPage2" to="/AboutMe"> Click here to get to know me better!</v-btn>
  </div>
</template>
  ...
  methods: {
    init() {
      ...
      for (let i = 0; i < this.NUM_INSTANCES; i++) {
        const { position, scaleX, scaleY, scaleZ } = this.instances[i];
        this.dummyO.position.copy(position);
        this.dummyO.scale.set(scaleX, scaleY, scaleZ);
    ...
    animate() {
      ...
      for (let i = 0; i < this.NUM_INSTANCES; i++) {
        const { position, scaleX, scaleY, scaleZ, velocity, attraction, vlimit } = this.instances[i];
